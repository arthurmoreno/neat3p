#!/usr/bin/env python3
"""
VoxelForage gate — Adaptive HyperNEAT (CPPN substrate net with online Hebbian plasticity).

Like the plain-HyperNEAT entry, but the CPPN also emits a ``delta_w`` rule that updates the
hidden weights each step (within-lifetime plasticity). Uses the shipping ``AdaptiveNet``
unchanged; ``VoxelForageAdaptiveNet`` only bakes the substrate coordinates.

Usage:
    python benchmarks/voxel_forage_adaptive_hyperneat.py
    python benchmarks/voxel_forage_adaptive_hyperneat.py --no-scent
    python benchmarks/voxel_forage_adaptive_hyperneat.py --render
"""
...
_CFG = os.path.abspath(os.path.join(os.path.dirname(__file__), "configs/voxel_forage_adaptive_hyperneat.cfg"))
_OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "output")

BENCHMARK_NAME = "voxel_forage_adaptive_hyperneat"
...
class VoxelForageAdaptiveNet:
    """Bakes the voxel substrate into the shipping AdaptiveNet (recurrent-style interface)."""
    ...
    def create(cls, genome, config, batch_size: int = 1, use_current_activs: bool = True, device: str = _DEVICE):
        net = AdaptiveNet.create(
            genome, config,
            input_coords=_INPUT, hidden_coords=_HIDDEN, output_coords=_OUTPUT,
...
def run_benchmark(
    ...
    winner_path = save_winner(save_dir or _OUTPUT_DIR, "adaptive_hyperneat", result, env_id, seed, _CFG)
    if verbose:
...
def _watch(winner, config, episodes, env_id):
    ...
    net = VoxelForageAdaptiveNet.create(winner, config)
...
def main():
    ...
    print(f"Training {env_id} with Adaptive HyperNEAT (seed={args.seed}, gens={args.generations})...")
    ...
    result = run_neat_gym(
        ...
        net_class=VoxelForageAdaptiveNet, verbose=True,
    )
