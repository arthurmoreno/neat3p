...
from neat3p.graphs import required_for_output
...
class OptimizedRecurrentNet:
    def __init__(
        ...
        self.transfer_stream = torch.cuda.Stream(device=self.device) if device.startswith("cuda") else None

        def async_to_device(tensor):
            if self.device.startswith("cuda"):
                return tensor.to(self.device, non_blocking=True)
            else:
                return tensor.to(self.device)

        context = torch.cuda.stream(self.transfer_stream) if self.transfer_stream else _dummy_context()

        with context:
            if n_hidden > 0:
                self.input_to_hidden = async_to_device(
                    dense_from_coo((n_hidden, n_inputs), input_to_hidden, dtype=dtype)
                )
                self.hidden_to_hidden = async_to_device(
                    dense_from_coo((n_hidden, n_hidden), hidden_to_hidden, dtype=dtype)
                )
                self.output_to_hidden = async_to_device(
                    dense_from_coo((n_hidden, n_outputs), output_to_hidden, dtype=dtype)
                )
                self.hidden_to_output = async_to_device(
                    dense_from_coo((n_outputs, n_hidden), hidden_to_output, dtype=dtype)
                )
            self.input_to_output = async_to_device(dense_from_coo((n_outputs, n_inputs), input_to_output, dtype=dtype))
            self.output_to_output = async_to_device(
                dense_from_coo((n_outputs, n_outputs), output_to_output, dtype=dtype)
            )
            if n_hidden > 0:
                self.hidden_responses = async_to_device(torch.tensor(hidden_responses, dtype=dtype))
                self.hidden_biases = async_to_device(torch.tensor(hidden_biases, dtype=dtype))
            self.output_responses = async_to_device(torch.tensor(output_responses, dtype=dtype))
            self.output_biases = async_to_device(torch.tensor(output_biases, dtype=dtype))
    ...
    def activate(self, inputs):
        if self.transfer_event and not self.transfer_event.query():
            raise RuntimeError("GPU transfers not complete yet!")

        with torch.no_grad():
            inputs_tensor = torch.tensor(inputs, dtype=self.dtype).to(self.device, non_blocking=True)
            inputs_t = inputs_tensor.t()

            activs_for_output = self.activs
            if self.n_hidden > 0:
                for _ in range(self.n_internal_steps):
                    temp = (
                        self.input_to_hidden.mm(inputs_t).t()
                        + self.hidden_to_hidden.mm(self.activs.t()).t()
                        + self.output_to_hidden.mm(self.outputs.t()).t()
                    )
                    self.activs = self.activation(self.hidden_responses * temp + self.hidden_biases)
                if self.use_current_activs:
                    activs_for_output = self.activs
            output_inputs = self.input_to_output.mm(inputs_t).t() + self.output_to_output.mm(self.outputs.t()).t()
            if self.n_hidden > 0:
                output_inputs += self.hidden_to_output.mm(activs_for_output.t()).t()
            self.outputs = self.activation(self.output_responses * output_inputs + self.output_biases)
        return self.outputs
